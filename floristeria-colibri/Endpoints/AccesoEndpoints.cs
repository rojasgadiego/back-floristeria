...
public class AccesoEndpoints : EndpointsBase
{
    ...
    public void MapEndpoints(IEndpointRouteBuilder app)
    {
        ...
        auth.MapPost("/login", Login)
            ...
            .AllowAnonymous()
            .Produces<ResponseDto>(200).Produces(401).Produces(400);
        ...
        usuarios.MapGet("/", ListarUsuarios)
            ...
            .Produces<ResponseDto>(200).Produces(401).Produces(403);

        usuarios.MapGet("/{id:int}", ObtenerUsuario)
            .WithName("ObtenerUsuario")
            .Produces<ResponseDto>(200).Produces(404).Produces(401).Produces(403);

        usuarios.MapPost("/", CrearUsuario)
            .WithName("CrearUsuario")
            .Produces<ResponseDto>(201).Produces(400).Produces(401).Produces(403);

        usuarios.MapPut("/{id:int}", ActualizarUsuario)
            .WithName("ActualizarUsuario")
            .Produces<ResponseDto>(200).Produces(400).Produces(401).Produces(403);
    ...
    public async Task<ResponseDto> Login(
        [FromBody] LoginRequest peticion, AccesoBLL bll, CancellationToken ct)
    {
        try
        {
            if (peticion is null)
        ...
        catch (Exception ex)
        {
            ...
            return CustomUtilz.CreateResponse(
                HttpStatusCodes.InternalServerError, "Error al iniciar sesión.", null);
        }
    }

    public async Task<ResponseDto> Yo(AccesoBLL bll, HttpContext http, CancellationToken ct)
        => await ConsultarUno(
            () => bll.Yo(UsuarioActual(http), ct), "Usuario", "La sesión ya no es válida.");

    public async Task<ResponseDto> CambiarMiPassword(
        [FromBody] CambiarPasswordRequest peticion, AccesoBLL bll, HttpContext http, CancellationToken ct)
    {
        try
        {
            ...
            return string.IsNullOrWhiteSpace(error)
                ? CustomUtilz.CreateResponse(HttpStatusCodes.Ok, "Contraseña actualizada", new { Cambiada = true })
                : CustomUtilz.CreateResponse(HttpStatusCodes.BadRequest, error, new { Cambiada = false });
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error al cambiar contraseña");
            return CustomUtilz.CreateResponse(
                HttpStatusCodes.InternalServerError, "Error al cambiar la contraseña.", null);
        }
    }
    ...
    public async Task<ResponseDto> ListarUsuarios(
        [FromQuery] string? busqueda, [FromQuery] RolUsuario? rol, [FromQuery] bool? activo,
        AccesoBLL bll, CancellationToken ct)
        => await Consultar(() => bll.ListarUsuarios(busqueda, rol, activo, ct), "usuarios");

    public async Task<ResponseDto> ObtenerUsuario(int id, AccesoBLL bll, CancellationToken ct)
        => await ConsultarUno(() => bll.ObtenerUsuario(id, ct), "Usuario", $"No existe el usuario {id}.");

    public async Task<ResponseDto> CrearUsuario(
        [FromBody] CrearUsuarioRequest peticion, AccesoBLL bll, CancellationToken ct)
    {
        try
        {
            ...
            return r.Ok
                ? CustomUtilz.CreateResponse(HttpStatusCodes.Created, $"{r.Datos!.Nombre} agregado", r.Datos)
        ...
        catch (Exception ex)
        {
            ...
            return CustomUtilz.CreateResponse(
                HttpStatusCodes.InternalServerError, $"Error al crear el usuario: {ex.Message}", null);
        }
    }

    public async Task<ResponseDto> ActualizarUsuario(
        int id, [FromBody] ActualizarUsuarioRequest peticion, AccesoBLL bll, CancellationToken ct)
    {
        try
        {
            ...
            return r.Ok
                ? CustomUtilz.CreateResponse(HttpStatusCodes.Ok, "Usuario actualizado", r.Datos)
                : CustomUtilz.CreateResponse(HttpStatusCodes.BadRequest, r.Mensaje, null);
        }
        catch (Exception ex)
        {
            ...
            return CustomUtilz.CreateResponse(
                HttpStatusCodes.InternalServerError, $"Error al actualizar el usuario: {ex.Message}", null);
        }
    }

    public async Task<ResponseDto> ResetearPassword(
        int id, [FromBody] ResetPasswordRequest peticion, AccesoBLL bll, CancellationToken ct)
    {
        try
        {
            ...
            return string.IsNullOrWhiteSpace(error)
                ? CustomUtilz.CreateResponse(HttpStatusCodes.Ok, "Contraseña restablecida", new { Cambiada = true })
                : CustomUtilz.CreateResponse(HttpStatusCodes.BadRequest, error, new { Cambiada = false });
        }
        catch (Exception ex)
        {
            ...
            return CustomUtilz.CreateResponse(
                HttpStatusCodes.InternalServerError, "Error al restablecer la contraseña.", null);
        }
    }

    public async Task<ResponseDto> CambiarEstado(
        int id, bool activo, AccesoBLL bll, HttpContext http, CancellationToken ct)
    {
        try
        {
            ...
            return r.Ok
                ? CustomUtilz.CreateResponse(
                    HttpStatusCodes.Ok, activo ? "Usuario activado" : "Usuario desactivado", r.Datos)
                : CustomUtilz.CreateResponse(HttpStatusCodes.BadRequest, r.Mensaje, null);
        }
        catch (Exception ex)
        {
            ...
            return CustomUtilz.CreateResponse(
                HttpStatusCodes.InternalServerError, "Error al cambiar el estado.", null);
        }
    }
